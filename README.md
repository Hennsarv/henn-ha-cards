...
- The **Layered editor** manages ordering, globals, layer sequence, wrapper style, resolution rules, and embedded cards. Add a layer with **+**, select a child card, and configure it with that card's own visual editor. If the child has no editor, Layered provides a JSON fallback. Use **↺** to replace a child card type.
